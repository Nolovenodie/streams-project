...
from ._utiles import cnv_options_to_args, get_path_info, clean_args, mkdir
...
def _get_hls_stream(hls, rep, dirname):
    """
    @TODO: add documentation
    """
    streamsPath = os.path.join(dirname, "streams", str(rep.size.height) + "p")
    mkdir(streamsPath)
    ...
    args.update({
        ...
        'hls_segment_filename':     os.path.join(streamsPath, "%d." + _hls_seg_ext(hls)),
        'hls_fmp4_init_filename':   os.path.join(streamsPath, "init.mp4"),
        's:v':                      rep.size,
    ...
    return cnv_options_to_args(args) + [streamsPath + ".m3u8"]
...
def _hls(hls):
    ...
    for key, rep in enumerate(hls.reps):
        ...
        streams += _get_hls_stream(hls, rep, hls.output_)
